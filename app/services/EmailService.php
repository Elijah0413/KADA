...
class EmailService
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            $this->mailer->isSMTP();
            $this->mailer->Host = 'mail.kadakeperasi.cc';
            $this->mailer->SMTPAuth = true;
            $this->mailer->Username = 'kadakepe@zen.ix-dns.com';  // Using the email from your webmail
            $this->mailer->Password = 'nQ_j-8M?7Mu'; // Make sure this is an App Password
            $this->mailer->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS; // Changed to SMTPS
            $this->mailer->Port = 465; // Changed to 465
            $this->mailer->CharSet = 'UTF-8';
            $this->mailer->isHTML(true);
            
            // Set default sender
            $this->mailer->setFrom('kadakeperasi@kadakeperasi.cc', 'KADA System');
            ...
            error_log("Debug info: " . $this->mailer->ErrorInfo);
        }
    }
    public function sendPasswordResetEmail($email, $token)
    {
        try {
            ...
            // Update to use your actual domain
            $resetLink = "https://kadakeperasi.cc/reset-password?token=" . $token;
            
            // Update to use the correct sender email
            $this->mailer->setFrom('kadakeperasi@kadakeperasi.cc', 'KADA System'); // Changed from elijahshe04@gmail.com
            $this->mailer->addAddress($email);
            $this->mailer->isHTML(true);
            $this->mailer->Subject = 'Reset Your Password';
            
            // Add more detailed error logging
            error_log("Attempting to send reset password email to: " . $email);
            error_log("Using reset link: " . $resetLink);
    ...
    public function sendVerificationEmail($email, $token)
    {
        try {
            ...
            // Update to use your actual domain and redirect to userlogin
            $verificationLink = "https://kadakeperasi.cc/verify-email?token=" . $token . "&redirect=userlogin";
