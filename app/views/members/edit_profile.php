...
<body>
                            ...
                            <div class="family-member-entry border rounded p-3 mb-3 bg-light">
                                <div class="row g-3">
                                    <div class="col-auto d-flex align-items-center">
                                        <span class="member-number badge bg-success rounded-circle"><?= $index + 1 ?></span>
                                    </div>
                                    <div class="col-md-4">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.remove-family-member').forEach(button => {
            button.addEventListener('click', function() {
                if (confirm('Adakah anda pasti ingin membuang ahli keluarga ini?')) {
                    ...
                    updateFamilyCount(); // Update count after removal
                }
            });
        });
                ...
                if (newRemoveBtn) {
                    newRemoveBtn.addEventListener('click', function() {
                        if (confirm('Adakah anda pasti ingin membuang ahli keluarga ini?')) {
                            this.closest('.family-member-entry').remove();
                            reindexFamilyMembers();
                            updateMemberNumbers(); // Update all member numbers
                            updateFamilyCount();
                        }
                    });
                }
        ...
        // Function to update member numbers
        function updateMemberNumbers() {
            document.querySelectorAll('.member-number').forEach((span, index) => {
                span.textContent = index + 1;
            });
        }

        // Add to existing remove button event listeners
        document.querySelectorAll('.remove-family-member').forEach(button => {
            button.addEventListener('click', function() {
                if (confirm('Adakah anda pasti ingin membuang ahli keluarga ini?')) {
                    this.closest('.family-member-entry').remove();
