...
class Member extends Model
{
    ...
    public function submitTerminationRequest($data)
    {
        try {
            ...
            if (!$success) {
                throw new Exception("Failed to insert termination request");
            }
            ...
            $this->db->rollBack();
            error_log("Database Error in submitTerminationRequest: " . $e->getMessage());
            throw new Exception("Ralat telah berlaku. Sila cuba lagi.");
        } catch (Exception $e) {
            // Rollback the transaction on error
            $this->db->rollBack();
            error_log("Error in submitTerminationRequest: " . $e->getMessage());
            throw new Exception("Ralat telah berlaku. Sila cuba lagi.");
        }
    }
    ...
    public function getTerminationRequest($icNo)
    {
        try {
            $stmt = $this->db->prepare("
                SELECT * FROM membership_termination 
                WHERE ic_no = ? 
                ORDER BY created_at DESC 
                LIMIT 1
            ");
            ...
            error_log("Error in getTerminationRequest: " . $e->getMessage());
            return false;
        }
    }
